...
const currentProjectModal = ({ project, show }) => {
  ...
  return (
    <button
      className="fixed left-0 right-0 top-0 bottom-0 w-full h-full z-50 bg-gray-500 bg-opacity-25"
      onClick={() => setShowModal(false)}
    >
      <div className="bg-white w-4/5 h-4/5 opacity-100 rounded-lg shadow-xl my-auto mx-auto dark:bg-slate-800">
        <div className="relative w-full p-3 border-b-2 text-xl">
          {project.title}
          <button
            className="absolute right-0 py-1 px-4"
            onClick={() => setShowModal(false)}
          >
            <FaTimes />
          </button>
